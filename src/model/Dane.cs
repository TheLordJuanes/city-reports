        ...
        public DataTable importCVSFile(string csv_file_path) {
            ...
            using (StreamReader csvReader = new StreamReader(csv_file_path)) {
                ...
                while (!csvReader.EndOfStream) {
                    ...
                    Department temp = existDepartment(fieldData[0]);
                    if (temp == null) {
                        departments.Add(depTemp);
                        departments.ElementAt(departments.Count-1).addMunicipality(munTemp);
                    } else
                        temp.addMunicipality(munTemp);
                }
                return csvData;
            }
        }

        public Department existDepartment(string depCode) {
